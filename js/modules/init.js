import {user, baseContainer} from './createElements.js';
import {getTaskData,
  removeTaskData,
  changeTaskData,
  editTaskData,
  data,
changeBackTaskData} from './serviceStorage.js';
...
const init = () => {
  ...
  list.addEventListener('click', e => {
    ...
    if (target.closest('.btn-success')) {
      target.closest('.task_row').classList.toggle('table-success');
      if (target.closest('.task_row').classList.contains('table-success') === true) {
        ...
        const task = (target.closest('.task_row').querySelector('.task').textContent);
        target.closest('.task_row').querySelector('.task_status').textContent = 'в процессе';
        changeTaskData(task);
      }
      // if (target.closest('.task_row').classList.contains('table-success') === true) {
      // } else {
